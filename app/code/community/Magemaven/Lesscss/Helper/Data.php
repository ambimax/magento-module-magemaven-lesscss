 ...
 */
require_once(Mage::getBaseDir('lib') . DS . 'lessphp' . DS . 'lessc.inc.php'); // @codingStandardsIgnoreLine
...
class Magemaven_Lesscss_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function compile($file)
    {
        ...
        try {
            ...
            if ( $cache ) {
                $cache = @unserialize($cache); // @codingStandardsIgnoreLine
            }

            if ( !file_exists($targetFilename) ) { // @codingStandardsIgnoreLine
                $cache = false;
            }
